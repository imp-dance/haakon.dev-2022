...
export default function usePagination(
  list: Array<any> | null = [{}],
  options: IOptions = { perPage: 10, searchDeps: [] }
): [Array<any>, IReturn] {
  const [activePage, setActivePage] = useState(1);
  const [pages, setPages] = useState([1]);
  const [filteredList, setFilteredList] = useState(list);
  const { perPage, onPageChange, isHidden, maxButtons } =
    options;
  ...
  useEffect(() => {
    if (list) {
      ...
      setFilteredList(
        parsedList.slice(activePage * pp - pp, activePage * pp)
      );

      const calculatePages = () => {
        const pp = perPage ?? 10;
        const amountOfPages: number = Math.ceil(
          parsedList.length / pp
        );
  ...
  }, [list, activePage, perPage, ...(options.searchDeps ?? [])]);
